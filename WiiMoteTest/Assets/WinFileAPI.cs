...
namespace Assets
{
    public class WinApiFile : IDisposable
    {
        ...
        public int FileLength
        {
            get
            {
                return (_hFile != null) ? (int)GetFileSize(_hFile, IntPtr.Zero) : 0;
            }
            set
            {
                if (_hFile == null)
        ...
        public static uint ReadFile(SafeFileHandle file, byte[] buffer, uint cbToRead)
        {
            // returns bytes read
            uint cbThatWereRead = 0;
            if (!ReadFile(file, buffer, cbToRead,
             ref cbThatWereRead, IntPtr.Zero))
                Debug.Log("AA");
            return cbThatWereRead;
        }
